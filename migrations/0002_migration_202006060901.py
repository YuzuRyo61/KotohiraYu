...
class known_users(peewee.Model):
    ...
    ID_Inst = IntegerField(index=True, unique=True)
...
@snapshot.append
class drill_count(peewee.Model):
    ID = AutoField(primary_key=True)
    ID_Inst = snapshot.ForeignKeyField(backref='drill_count', index=True, model='known_users', unique=True)
    tootCount = CharField(max_length=32)
    class Meta:
        table_name = "drill_count"
